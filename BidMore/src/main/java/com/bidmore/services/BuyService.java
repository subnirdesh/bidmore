...
public class BuyService {
	...
	public AuctionModel getAuctionById(int auctionId) {
		...
		// Query to fetch specific auction info using join queries and auction_id
		// parameter
		String query = "SELECT a.auction_id, a.start_price, a.reserve_price, a.end_time, "
				+ "i.item_id, i.name, i.description, i.condition, i.category, i.image_path, "
				+ "u.user_id, u.user_name, u.first_name, u.last_name " + "FROM auctions a"
	...
	public Boolean placeBid(BidModel bid) {
		...
		// Insert the bid
		String insertQuery = "INSERT INTO bids ( bidder_id, auction_id,bid_amount) " + "VALUES (?, ?, ?)";
	...
	public List<AuctionModel> getListingsFromSearch(String searchTerm) {
		/*
		 * Checking for database connection; in case of connection error returning null
		 */
		if (isConnectionError) {
		...
		// Create search pattern - default to fetching all if no search term provided
		String searchPattern = "%";
		if (searchTerm != null && !searchTerm.isEmpty()) {
			searchPattern = searchTerm.charAt(0) + "%"; // First alphabet followed by wildcard
		}

		// Query to fetch item info using join queries with search condition
		String query = "SELECT a.auction_id, a.start_price, a.reserve_price, a.end_time, "
				+ "i.item_id, i.name, i.description, i.condition, i.category, i.image_path, "
				+ "u.user_id, u.user_name, u.first_name, u.last_name " + "FROM auctions a"
				+ " JOIN items i ON a.item_id = i.item_id" + " JOIN users u ON i.seller_id = u.user_id "
	...
	public List<AuctionModel> getListingsByCategory(String category) {
		/*
		 * Checking for database connection; in case of connection error returning null
		 */
		if (isConnectionError) {
		...
		// Query to fetch item info using join queries, filtered by category
		String query = "SELECT a.auction_id, a.start_price, a.reserve_price, a.end_time, "
				+ "i.item_id, i.name, i.description, i.condition, i.category, i.image_path, "
				+ "u.user_id, u.user_name, u.first_name, u.last_name " + "FROM auctions a"
				+ " JOIN items i ON a.item_id = i.item_id" + " JOIN users u ON i.seller_id = u.user_id "
